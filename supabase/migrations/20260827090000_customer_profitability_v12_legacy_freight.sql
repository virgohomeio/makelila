-- Backlog #58 V12 -- the freight that predates Freightcom.
--
-- VCycene shipped via Canpar, GLS, Purolator and FedEx from Oct 2025 until the
-- Freightcom cutover on 2026-01-26. None of it is in `shipments`, and the
-- Freightcom API has no record of it, so 44 real shipments worth $2,526.81 CAD
-- sat outside the margin entirely.
--
-- Attributed per CUSTOMER, not per order. Every row matched a customer on exact
-- email and 42 of 42 serials matched a unit, but ZERO carried an order link --
-- most of that Oct/Nov 2025 cohort predates any order record in this system.
-- Pinning the cost to an order would mean inventing the order.
--
-- One row is excluded: Elizabeth Antony's $66.01 (D420352470001668407001) is
-- already on order #1173 as shipping_cost_basis = 'legacy_backfill'. It stays
-- in legacy_shipping_costs for the audit trail with superseded_by_order_ref
-- set, and out of the sum. $2,460.80 is countable.
...
comment on view public.customer_profitability is
  'Backlog #58 V12: 10-bucket contribution margin in CAD. Freight now has three '
  'sources -- Freightcom invoices (billed, after adjustments), the synced booking '
  'quote, and pre-Freightcom Canpar/GLS/Purolator/FedEx costs attributed per '
  'customer. CAC and LTV are allocated in lib/profitability.ts, not here.';
