-- Seed customer return: Katrina Dowd (order #1104, Florida resident).
--
-- Mirrors Ron Russell's seed (20260421120000) with one key difference: by
...
-- refine the auto-created note with Katrina-specific context so the manager
-- sees the same quality of triage detail Ron's row has.
--
...
— Refund preference: Email (E-Transfer) to Katrinadowd83@gmail.com
...
where not exists (
  select 1 from public.returns
   where customer_email = 'Katrinadowd83@gmail.com'
     and original_order_ref = '#1104'
);

-- Refine the auto-created refund_approvals note (the trigger writes a
-- generic message; this gives the manager Katrina-specific context).
update public.refund_approvals ra
   set notes = 'Customer-form submission. Katrina requested E-Transfer refund to Katrinadowd83@gmail.com. Customer paid via Sezzle — verify refund routing with finance before issuing. Manager: confirm $1049 list-price amount (LILA Pro).'
  from public.returns r
 where ra.return_id = r.id
   and r.customer_email = 'Katrinadowd83@gmail.com'
