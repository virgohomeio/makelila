-- Backlog #58 V11 -- two cost sources the model could not see.
--
-- 1. FREIGHT WAS THE QUOTE, NOT THE BILL. orders.shipping_cost_usd came from
--    shipments.billed_amount, which Freightcom sets at booking and never
--    revises. Four batch fuel-surcharge invoices (FC15377901, FC15377903,
--    FC15539049, FC15741148) raised 30 of 126 shipments after the fact --
--    +$391.26 that never reached a margin. The view now prefers
--    shipment_invoiced_charges.applicable_cad, which is net of both later
--    adjustments and refunds, and falls back to the quote when no invoice is
--    on file.
--
-- 2. CONSUMABLES WERE NOWHERE. 78 Amazon orders (worm castings, and two
--    jumper-cap kits) shipped to named customers, CAD 1,811. These were asked
--    for as "Amazon shipping costs", but the money buys product the customer
--    keeps -- freight on them was free with Prime. Filing them under Shipping
--    would have made the freight-per-unit metrics wrong, so they are their own
--    bucket, next to cost of goods where they belong.
--
-- Both are additive: nothing already counted moves bucket.
...
comment on view public.customer_profitability is
  'Backlog #58 V11: 10-bucket contribution margin in CAD (COGS, freight, warranty, '
  'refunds, diagnosis-call labour, return handling, payment fees, sales commission, '
  'installation, consumables/parts) plus province/state, acquisition channel and '
  'cohort anchor. Freight prefers shipment_invoiced_charges (billed, after '
  'adjustments) over the synced quote. CAC and LTV are allocated in '
