-- Backlog #58 V10 -- the freight gap was under-reporting itself by 94%.
--
-- V7 scoped shipping_uncosted_count to orders that had shipped, which was the
-- right idea: an order still in the queue owes no freight, and counting it as
-- a gap would cry wolf on every open order. The test it used to answer
-- "did this ship" was units.customer_order_ref = orders.order_ref.
--
-- That column is 55% empty. Of 176 shipped units only 80 carry an order ref,
-- so the view flagged 3 uncosted orders out of 104 with no freight cost, and
-- costCoverage() told operators the margin was complete for the other 101.
--
-- units.customer_id carries 158 of the 176. Asking the question there raises
-- the flag from 3 to 50 -- roughly $7.5k of freight at the $151 average of the
-- 113 costed orders. Nothing here invents a cost: the dollars stay absent and
-- the margin stays an upper bound. The only change is that the gap now says
-- how big it is.
--
-- Not fixable in SQL: ~30 of those 50 orders predate Freightcom (first
-- shipment 2026-01-26) and went out via Canpar/GLS. No table in this database
-- holds their freight -- it needs a manual backfill from the carrier invoices
-- with shipping_cost_basis = 'legacy_backfill'.
--
-- Only the view changes; normalize_region() and normalize_channel() are
-- untouched from V9.
...
order_agg as (
  select
    ...
    -- that is a separate question, tracked on its own.)
    count(*) filter (where order_id is not null and kind = 'sale'
                     and shipping_cost_usd is null and order_shipped
...
comment on view public.customer_profitability is
  'Backlog #58 V10: 9-bucket contribution margin in CAD (COGS, freight, warranty, '
  ...
  'shipping_uncosted_count asks the shipped question at customer level, not order '
  'level, and excludes cancelled orders. CAC and LTV are allocated in '
  'lib/profitability.ts, not here.';
