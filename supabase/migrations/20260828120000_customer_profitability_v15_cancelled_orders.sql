-- Backlog #58 V15 -- a cancelled sale order stops being a customer's problem.
--
-- DECISION (Huayi, 2026-08-28). When an order is cancelled the customer's
-- account is credited and LILA keeps the machine. So:
--
--   * No revenue. There was no sale. Twenty cancelled sale orders were
--     contributing $9,911.96 of revenue the business never collected.
--   * No customer-level cost. The unit went back to stock, so charging its
--     $14,777.76 of COGS to the person who cancelled makes them look like the
--     worst customer in the book for an order they did not complete.
--   * The cost does not disappear. The machines were built and the money was
--     spent. It moves to public.retained_unit_costs -- a company-level line
--     carried by nobody in particular, surfaced on the tab OUTSIDE
--     contribution margin.
--
-- This finally matches how replacement orders have always been treated: a
-- cancelled replacement has never counted toward warranty cost. The same
-- condition was being handled two different ways.
--
-- FREIGHT GOES WITH IT. Four of the twenty shipped before they were cancelled,
-- costing $551.16 that was genuinely spent. It leaves the customer's margin
-- along with the order and lands on the retained-unit line -- dropping it
-- would understate what a cancellation actually costs.
--
-- REFUNDS STAY WHERE THEY ARE, AND THAT IS A KNOWN PROBLEM. An approval is
-- matched to a customer directly rather than through an order, so a refunded
-- cancellation still carries its refund as a cost -- against revenue this
-- migration just removed. That is the reversal counted twice.
--
-- Five customers are affected, all of whose only sale order was cancelled:
-- Katrina Dowd, Ron Russell and Phayvanh Nanthavongdouangsy at $1,456.94,
-- $1,456.94 and $1,411.61, plus Gabriella Hottya and Marina Weiss at $1,111.10
-- each -- $6,547.69 of refund now sitting against $0 of revenue. Their margins
-- overstate the loss by roughly that much.
--
-- Not fixed here because it cannot be fixed reliably: refund_approvals.order_id
-- is null on all 18 rows, so there is no link from a refund to the order it
-- reversed. Matching on customer name plus amount is a judgement call, and the
-- amounts do not tie out -- Ron Russell's refund is $1,456.94 against a $999.99
-- order, so at least some refunds span more than the cancelled order alone.
-- Needs a decision and probably needs order_id backfilled first.
--
-- All twenty cancelled orders are "1x LILA Pro", so each one really is a
-- retained machine rather than an accessory line.
...
order_match as (
  select
    ...
    -- V15. Cancelled means the account was credited and the unit came back to
    -- stock. Every sale aggregate below excludes these; their cost is carried
    -- company-wide by public.retained_unit_costs instead.
...
first_sale as (
  ...
  from order_match
  where order_id is not null and kind = 'sale' and not order_cancelled
...
order_agg as (
  select
    ...
    coalesce(sum(public.to_cad(total_usd - coalesce(tax_usd, 0), currency))
             filter (where kind = 'sale' and not order_cancelled), 0)::numeric(12,2) as revenue_cad,
    coalesce(sum(public.to_cad(coalesce(tax_usd, 0), currency))
             filter (where kind = 'sale' and not order_cancelled), 0)::numeric(12,2) as tax_collected_cad,
    ...
    coalesce(sum(public.to_cad(coalesce(discount_total_usd, 0), currency))
             filter (where kind = 'sale' and not order_cancelled), 0)::numeric(12,2) as discount_cad,
    -- Processors charge on the amount actually collected, tax included.
    coalesce(sum(public.to_cad(total_usd, currency))
             filter (where kind = 'sale' and not order_cancelled), 0)::numeric(12,2) as charged_gross_cad,
    coalesce(sum(public.to_cad(cogs_usd, 'USD'))
             filter (where kind = 'sale' and not order_cancelled), 0)::numeric(12,2) as sale_cogs_cad,
    ...
    )) filter (where kind = 'sale' and not order_cancelled), 0)::numeric(12,2)     as sale_shipping_cad,
    ...
    max(placed_at) filter (where kind = 'sale' and not order_cancelled)            as last_order_at,
    count(*) filter (where order_id is not null and kind = 'sale'
                     and not order_cancelled)                                      as order_count,
    -- Strict trace only: this is shown to operators as "Units shipped".
    count(*) filter (where order_id is not null and kind = 'sale'
                     and not order_cancelled and order_traced)                     as units_shipped_count,
    ...
    -- sit in the gap. As of V15 it contributes no revenue or COGS either, so
    -- the two treatments finally agree. An order with an invoice on file is
    -- costed even when the synced quote is null.
    count(*) filter (where order_id is not null and kind = 'sale'
...
comment on view public.customer_profitability is
  'Backlog #58 V15: 11-bucket contribution margin in CAD. A cancelled sale order no longer '
  'contributes revenue, COGS, freight, discount, tax or order count to any customer -- the '
  'account was credited and the unit retained, so its cost moves to '
  'public.retained_unit_costs and is reported company-wide instead. Matches the treatment '
  'cancelled replacement orders have always had.';


-- ---------------------------------------------------------------------------
-- Where a cancelled order's cost goes.
--
-- One row per cancelled sale order: the machine that was built and kept, plus
-- the freight if it had already gone out. No customer_id on purpose -- the
-- whole point of V15 is that nobody in particular carries this. The tab reads
-- it as one company-level line beneath contribution margin.
--
...
comment on view public.retained_unit_costs is
  ...
  'no customer -- it is reported company-wide, outside contribution margin.';
