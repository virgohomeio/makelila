...
// Every other row in this table arrives from a sync (HubSpot, Shopify orders, a
// backfill script). This is the one path where an operator's typing goes
// straight in — a phone lead, a trade-show contact, a machine sold outside
// Shopify. The normalizer below is pure so the rules that keep a hand-typed row
// indistinguishable from a synced one are testable without a database.
// Spec: docs/superpowers/specs/2026-09-24-manual-add-customer-design.md

/** The Add-customer form's fields. All strings — the form has no null state. */
...
/** A blank draft. Exported so the form and its tests share one definition of
 *  "empty" and can't drift a field apart. */
...
/** What actually gets inserted. Note the absence of full_name: it is
 *  GENERATED ALWAYS AS (trim(first_name || ' ' || last_name)) and writing it is
 *  an error. last_synced_at is absent too — this row was never synced, and
 *  claiming otherwise would make the header's sync date a lie. */
...
/** Trim, lowercase the email, turn blanks into NULL, and reject the two inputs
 *  that make an unusable record: no name at all, and a mistyped email.
 *  Throws with a message meant to be shown to the operator verbatim. */
export function normalizeNewCustomer(input: NewCustomerInput): NewCustomerRow {
  const first = blankToNull(input.first_name);
  const last = blankToNull(input.last_name);
  if (!first && !last) {
    // The directory is sorted and searched by name; a row whose generated
    // full_name is '' can be created but never found again.
    throw new Error('Enter a name — a customer with no name can\'t be found in the directory.');
  }
  ...
  return {
    // Deliberately no split of a compound name across first/last. Many synced
    // rows carry the whole thing in first_name ("James & Jill Washington"), and
    // guessing a surname out of one doubles it on display.
    first_name: first,
    last_name: last,
    email,
    phone: blankToNull(input.phone),
    address_line: blankToNull(input.address_line),
    city: blankToNull(input.city),
    region: blankToNull(input.region),
    postal_code: blankToNull(input.postal_code),
    country: blankToNull(input.country),
    notes: blankToNull(input.notes),
  };
}

/** Create a customer by hand from the Directory. Returns the new row's id so
 *  the caller can open its panel.
 *
 *  The email clash is checked here rather than left to the unique index on
 *  customers.email, for the same reason updateCustomerContact checks it: email
 *  is the key orders, tickets and refund cards match on, so the useful thing to
 *  say is WHO already has it, not "duplicate key value violates...". */
export async function createCustomer(input: NewCustomerInput): Promise<string> {
  ...
  if (row.email) {
    ...
    if (other) {
      throw new Error(
        `${other.full_name || 'Another customer'} already uses ${row.email}. ` +
        `Two customers can't share an email — orders and tickets are matched on it. ` +
        `Open that record instead, or link this person to them as a household user.`,
      );
    }
  }
  ...
  await logAction('customer_created', created.id, created.full_name,
