...
- **#74** Verify the diagnosis-call booking link's event-type before wiring #75's auto-invite. — **SHIPPED** (2026-06-04: rolled into #75's edge function `sync-google-appointments`. Uses a configurable `DIAGNOSIS_SUMMARY_PATTERN` regex (default `diagnosis`) + has-external-attendee filter to identify diagnosis bookings, plus a `DIAGNOSIS_PROBE_LOG=true` flag that emits the raw Calendar API event JSON for the first matches so the pattern can be tightened once we see real bookings. PATCHes the event to add `DIAGNOSIS_COHOST_EMAILS` co-hosts (Reina + Junaid) with `sendUpdates=all`, deduped via `service_tickets.diag_cohost_invited_at`.)
