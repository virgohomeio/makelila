...
- **2026-06-07 — activity_log entity refs landed** (commits `8d7f630` → `7326f64`, migration `20260607030000_activity_log_entity_refs.sql`). `activity_log` now has typed columns `entity_type` / `entity_id` / `unit_serial` with partial indexes. **For your OKR & KPI tracking (Feature 4):** the `okr_progress` view can now aggregate cheaply over typed columns (`where entity_type = 'return' and ts between ...`) instead of LIKE-matching across `detail`. The new `useActivityForEntity` hook in [lib/activityLog.ts](../../app/src/lib/activityLog.ts) is also the right call for any per-thing drilldown card on an OKR. Already-opted-in return-scoped events: `return_status`, `return_category` (entityType='return', entityId=return.id). Ticket events also carry entity refs when you want to count ticket lifecycle in an individual KPI.
